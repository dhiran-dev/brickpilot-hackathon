...
type RenderState = {
  ...
  assets: Array<{ id: string; role: "exterior_front" | "exterior_collage" | "exterior_top" | "interior" | string; url: string; contentType: string; index: number; schemeId: string | null }>;
  previousAssets?: PreviousSchemeRenderAsset[];
...
export function MassingWorkspace({ layoutVersionId, userName }: { layoutVersionId: string; userName: string }) {
  ...
  const [renderState, setRenderState] = useState<RenderState>({ status: "idle", jobs: [], assets: [], previousAssets: [], sources: [] });
  ...
  const layerControls = [
    ...
    { label: "Presentation", checked: presentationMode, setChecked: setPresentationMode, Icon: Presentation },
  ];
  ...
  async function prepareReferences() {
    ...
    try {
      ...
      flushSync(() => {
        ...
        setPresentationMode(MASSING_CAPTURE_LAYER_STATE.presentationMode);
      });
      await nextFrame();
      await nextFrame();
    ...
    try {
      const response = await fetch(`/api/designs/${layoutVersionId}/renders`, {
        ...
        body: JSON.stringify({
          ...
          schemeId: study.selectedSchemeId,
          selectedInteriorSpaceId,
          references,
        }),
      });
  ...
  return <main className="min-h-screen bg-[#090908] text-[#fff6ea]">
    ...
    <div className="mx-auto w-full max-w-[112rem] px-3 py-3 lg:px-5">
      ...
      <section className="overflow-hidden border border-[#8e5a31]/55 bg-[#0c0b09] shadow-[9px_10px_0_rgba(3,3,2,0.72)]">
        ...
        <div className="grid min-h-[44rem] min-[72rem]:grid-cols-[15.5rem_minmax(0,1fr)_20rem]">
          ...
          <div className="relative min-h-[44rem] overflow-hidden bg-[#080807]">
            <MassingViewer building={study.building} explodeM={explodeM} onError={setViewerError} onReadyChange={setViewerReady} presentationMode={presentationMode} ref={viewerRef} showColumns={showColumns} showInteriorWalls={showInteriorWalls} showRoof={showRoof} showSite={showSite} showSlabs={showSlabs} visibleFloorIds={visibleFloorIds} />
        ...
        <div className="grid gap-4 border-t border-[#8e5a31]/50 bg-[#0a0908] p-5 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
          {renderState.status !== "idle" || actionError ? <div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center border border-[#8e5a31]/65"><LockKeyhole className="h-5 w-5 text-[#c97940]" /></span><div>{renderState.status !== "idle" ? <><p className="font-[family-name:var(--font-display)] text-xl">{renderState.status === "processing" ? "GPT Image 2 is creating the grounded concepts." : renderState.status === "completed" ? "All four grounded concepts are ready." : "The massing remains available while renders are recovered."}</p><p className="mt-1 text-xs leading-5 text-[#8f8275]">{renderState.status === "processing" ? "This page can be left and reopened; status is persisted and reconciled." : "Concept visualization only. Geometry remains authoritative in the 2D and massing views."}</p></> : null}{actionError ? <p className="mt-2 text-xs text-[#ff806f]">{actionError}</p> : null}</div></div> : null}
      ...
      {renderState.status !== "idle" ? <section className="mt-8 border border-[#8e5a31]/55 bg-[#0c0b09] p-5" id="render-gallery"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#8e5a31]/40 pb-4"><div><p className="text-[0.61rem] font-extrabold uppercase tracking-[0.15em] text-[#c97940]">Step 4 · Render</p><h2 className="mt-1 font-[family-name:var(--font-display)] text-3xl">Concept renders grounded in your plan</h2></div><p className="max-w-xl text-xs leading-5 text-[#786d62]">Materials, furnishing and lighting are generative assumptions. Floor count, footprint and openings are constrained by the canonical references.</p></div>
        ...
        ] as const).map(([role, label]) => { const asset = assetsByRole.get(role); const sourceRole = role === "exterior_front" ? "massing_front" : role === "exterior_collage" ? "massing_collage" : role === "exterior_top" ? "massing_top" : "plan_reference"; const source = sourcesByRole.get(sourceRole); const schemeLabel = asset?.schemeId ? schemeNameById.get(asset.schemeId) ?? asset.schemeId : "Legacy scheme"; return <article className="border border-[#8e5a31]/45 bg-[#090908]" data-scheme-id={asset?.schemeId ?? "legacy"} key={role}><div className="relative aspect-[3/2] overflow-hidden">{asset ? <img alt={`${label} · ${schemeLabel}`} className="h-full w-full object-cover" loading="lazy" src={asset.url} /> : <div className="grid h-full place-items-center bg-[#0b0a09]"><span className="text-[0.61rem] font-bold uppercase tracking-[0.13em] text-[#695d53]">{label} · {renderState.status === "failed" ? "unavailable" : "rendering"}</span></div>}<span className="absolute left-3 top-3 bg-[#090908]/90 px-2 py-1 text-[0.52rem] font-bold uppercase tracking-[0.1em] text-[#fff6ea]">{label}{asset ? ` · ${schemeLabel}` : ""}</span></div><div className="grid grid-cols-[7rem_1fr] items-center gap-3 border-t border-[#8e5a31]/35 p-2.5">{source ? <img alt={`${label} exact submitted source`} className="aspect-[3/2] w-full border border-[#8e5a31]/35 object-cover" src={source.url} /> : <div className="grid aspect-[3/2] place-items-center border border-[#8e5a31]/25"><Camera className="h-3.5 w-3.5 text-[#574d45]" /></div>}<p className="text-[0.56rem] leading-4 text-[#756a60]"><strong className="block uppercase tracking-[0.09em] text-[#a99a8d]">Exact submitted source</strong>Camera and geometry are locked to this canonical reference; only materials, shallow elevation treatment, lighting and landscape may change.</p></div></article>; })}</div>
