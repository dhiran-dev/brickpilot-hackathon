    ...
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#090908] px-4 py-8">
      <div className="flex w-full max-w-[1360px] items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 px-4 py-2.5 text-[0.68rem] font-bold uppercase tracking-[0.11em] transition-colors ${downloadDisabled ? "cursor-not-allowed bg-[#4d2515] text-[#957461]" : "bg-[#ff4e00] text-[#fff6ea] hover:bg-[#e94500]"}`}
            href={downloadDisabled ? undefined : downloadHref}
            onClick={downloadDisabled ? (event) => event.preventDefault() : undefined}
            title={downloadDisabled ? "PDF export unlocks once every render has finished" : "Download the full sheet set as a vector PDF"}
          >
      ...
      <div className="flex w-full max-w-[1360px] gap-1.5 overflow-x-auto pb-1" ref={railRef}>
        {slideTitles.map((title, index) => (
          <button
            className={`flex shrink-0 flex-col items-center justify-center gap-0.5 border px-3 py-2 font-[family-name:var(--font-body)] text-[0.56rem] uppercase tracking-[0.06em] transition-colors ${index === activeIndex ? "border-[#ff4e00] text-[#fff6ea]" : "border-[#8e5a31]/40 text-[#b5a697] hover:border-[#c97940] hover:text-[#fff6ea]"}`}
            key={title + index}
            onClick={() => onNavigate(index)}
            type="button"
          >
            <b className={`font-[family-name:var(--font-display)] text-sm font-normal [font-variant-numeric:tabular-nums] ${index === activeIndex ? "text-[#ff4e00]" : ""}`}>{String(index + 1).padStart(2, "0")}</b>
