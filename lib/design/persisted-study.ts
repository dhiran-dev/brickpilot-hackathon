...
export type IncompatibleStudy = Pick<PersistedStudyRow, "projectId" | "designId" | "title" | "status" | "createdAt"> & {
  compatibility: "legacy_incompatible";
  reason: "INVALID_REQUIREMENTS" | "INVALID_BUILDING" | "INVALID_VALIDATION" | "INVALID_COST_ESTIMATE";
};
...
export function classifyPersistedStudy(row: PersistedStudyRow) {
  ...
  if (!requirements.success) return {
    ...
    study: { projectId: row.projectId, designId: row.designId, title: row.title, status: row.status, createdAt: row.createdAt, compatibility: "legacy_incompatible" as const, reason: "INVALID_REQUIREMENTS" as const },
  };

  if (row.status !== "completed") return {
    compatible: true as const,
    study: { ...row, requirements: requirements.data, building: null, validation: null, costEstimate: null },
  };

  const building = buildingSchema.safeParse(row.building);
  if (!building.success) return {
    compatible: false as const,
    study: { projectId: row.projectId, designId: row.designId, title: row.title, status: row.status, createdAt: row.createdAt, compatibility: "legacy_incompatible" as const, reason: "INVALID_BUILDING" as const },
  };
  const validation = persistedValidationReportSchema.safeParse(row.validation);
  if (!validation.success) return {
    compatible: false as const,
    study: { projectId: row.projectId, designId: row.designId, title: row.title, status: row.status, createdAt: row.createdAt, compatibility: "legacy_incompatible" as const, reason: "INVALID_VALIDATION" as const },
  };
  const costEstimate = costEstimateSchema.safeParse(row.costEstimate);
  if (!costEstimate.success) return {
    compatible: false as const,
    study: { projectId: row.projectId, designId: row.designId, title: row.title, status: row.status, createdAt: row.createdAt, compatibility: "legacy_incompatible" as const, reason: "INVALID_COST_ESTIMATE" as const },
  };
  return {
    compatible: true as const,
    study: { ...row, requirements: requirements.data, building: building.data, validation: validation.data, costEstimate: costEstimate.data },
  };
}
